// The machine VOIE_2_to_monte_charge_rangee1 that handle the push to foreight elevator event
public class VOIE_2_to_monte_charge_rangee1{
  
  public boolean guard_VOIE_2_to_monte_charge_rangee1() {
    // check the evnet condition
		return ((!Voie_arrive_2.apply(new Integer(0)).equals(none)) && (Borne_2.equals(true)) && (Monte_charge_etage.equals(new Integer(1))) && ((new Integer(Test_full_rangee1 + value_vehicules.apply(Voie_arrive_2.apply(new Integer(0))))).compareTo(new Integer(3)) <= 0) && (Barriere_acces_monte_charge.equals(true)) && (Barriere_monte_charge.apply(new Integer(1)).equals(true)));
	}

	public void run_VOIE_2_to_monte_charge_rangee1(){
    // run and animate the event
    if (ACTION == false && SelectEvent == 28){
    ...
    else if (ACTION == true && SelectEvent == 28){
